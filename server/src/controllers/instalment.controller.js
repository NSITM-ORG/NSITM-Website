...
const getOutstandingInstalments = asyncHandler(async (req, res, next) => {
  ...
  const filter = {
    status: mongoose.trusted({
      $in: [INSTALMENT_STATUS.NOT_SUBMITTED, INSTALMENT_STATUS.REJECTED],
    }),
    instalmentNumber: mongoose.trusted({ $in: [2, 3] }),
  };

  const [total, records] = await Promise.all([
    // InstalmentPayment.countDocuments()
    //   .where('status').in([INSTALMENT_STATUS.NOT_SUBMITTED, INSTALMENT_STATUS.REJECTED])
    //   .where('instalmentNumber').in([2, 3]),
    // InstalmentPayment.find()
    //   .where('status').in([INSTALMENT_STATUS.NOT_SUBMITTED, INSTALMENT_STATUS.REJECTED])
    //   .where('instalmentNumber').in([2, 3])
    ...
    InstalmentPayment.find(filter)
      ...
      .skip(skip)
      .limit(limit)
      .lean({ virtuals: true }), // virtuals still available with lean
  ]);
