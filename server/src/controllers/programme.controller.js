...
import { formatDurationString, isValidDurationValue } from '../helpers/durationParser.helper.js'
...
const createProgramme = asyncHandler(async (req, res, next) => {
  const { durationValue, durationUnit, ...rest } = req.body;

  if (!isValidDurationValue(durationValue, durationUnit)) {
    return next(new ApiError(HTTP_STATUS.BAD_REQUEST, 'INVALID_DURATION', `Duration value is invalid for the selected unit.`));
  }

  const programmeData = {
    ...rest,
    duration: formatDurationString(durationValue, durationUnit),
...
const updateProgramme = asyncHandler(async (req, res, next) => {
  ...
  // Client Issue 1 fix: subDescription and bulletPoints were omitted
  // from this list — they existed on the schema (F13) but silently
  // discarded on every edit request since this array is the whitelist
  // gate for what updateProgramme is permitted to touch.
  const allowedFields = [
    'name', 'category', 'subCategory', 'description', 'subDescription',
    'bulletPoints', 'prerequisites', 'status', 'fees', 'sortOrder', 'activeCohort',
  ];
  ...
  // Duration composed separately — same structured-input rule as create.
  if (req.body.durationValue !== undefined && req.body.durationUnit !== undefined) {
    if (!isValidDurationValue(req.body.durationValue, req.body.durationUnit)) {
      return next(new ApiError(HTTP_STATUS.BAD_REQUEST, 'INVALID_DURATION', `Duration value is invalid for the selected unit.`));
    }
    programme.duration = formatDurationString(req.body.durationValue, req.body.durationUnit);
  }
...
const bulkUpdateProgrammes = asyncHandler(async (req, res, next) => {
  ...
  // Validate + pre-compose the duration string ONCE, before the loop —
  // avoids re-validating the same pair on every iteration and matches
  // the exact fail-fast pattern already used in updateProgramme above.
  let bulkDurationString = null;
  if (updates.durationValue !== undefined && updates.durationUnit !== undefined) {
    if (!isValidDurationValue(updates.durationValue, updates.durationUnit)) {
      return next(new ApiError(HTTP_STATUS.BAD_REQUEST, 'INVALID_DURATION', 'Bulk duration value is invalid for the selected unit.'));
    }
    bulkDurationString = formatDurationString(updates.durationValue, updates.durationUnit);
  }
  ...
  for (const programme of programmes) {
    ...
    if (bulkDurationString) programme.duration = bulkDurationString;
