...
export const bulkUpdateProgrammes = createAsyncThunk(
  'programmes/bulkUpdate',
  async ({ ids, updates }, { rejectWithValue }) => {
    try {
      return (await httpClient.patch(API.SUPERADMIN.PROGRAMMES_BULK_UPDATE, { ids, updates })).data;
    } catch (err) {
      return rejectWithValue(err);
    }
  }
...
export const bulkDeleteProgrammes = createAsyncThunk(
  'programmes/bulkDelete',
  async (ids, { rejectWithValue }) => {
    try {
      return (await httpClient.del(API.SUPERADMIN.PROGRAMMES_BULK_DELETE, { ids })).data;
    } catch (err) {
      return rejectWithValue(err);
    }
  }
...
const programmeSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(bulkUpdateProgrammes.pending, (state) => {
        state.submitting = true;
      })
      .addCase(bulkUpdateProgrammes.fulfilled, (state) => {
        state.submitting = false;
      })
      .addCase(bulkUpdateProgrammes.rejected, (state, action) => {
        state.submitting = false;
        state.error = action.payload;
      })
      .addCase(bulkDeleteProgrammes.fulfilled, (state, action) => {
        const deletedIds = new Set(action.payload.deleted.map((d) => d.id));
        state.adminList = state.adminList.filter((p) => !deletedIds.has(p.id));
      });
  },
});
